...
public:
    ...
    void invalidImage () {
        my_puts("涓嶆槸涓€涓悎娉曠殑 FAT12 1.44M 杞洏鏂囦欢锛岀▼搴忛€€鍑猴紒");
        exit(-1);
    }
        ...
        if (args[0] == "exit") {
            if (options.size() == 0) {
                if (args.size() == 1) {
                    ...
                    my_puts("exit 涓嶈兘鏈夊弬鏁板摝锛乗n");
                }
            }
            else {
                my_puts("exit 涓嶈兘鏈夐€夐」鍝︼紒\n");
            }
        }
        else if (args[0] == "cat") {
            if (options.size() == 0) {
                if (args.size() == 2) {
                    ...
                    my_puts("cat 瑕佷笖浠呰涓€涓弬鏁板摝锛乗n");
                }
            }
            else {
                my_puts("cat 涓嶈兘鏈夐€夐」鍝︼紒\n");
            }
        }
        else if (args[0] == "ls") {
            if (options.size() > 1 || (options.size() == 1 && options[0] != 'l')) {
                my_puts("杩欎簺閫夐」淇轰笉璁よ瘑锛?);
